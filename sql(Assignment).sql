...
/*  Question 1 : Write a SQL query to find customers who are either from the city 'New York' or
...
/* Question 2 : Write a SQL query to find all the customers in ‘New York’ city who have a grade
...
/*Question 3: Write a SQL query that displays order number, purchase amount, and the
...
/* Question 4 : Write a SQL query to calculate the total purchase amount of all orders. Return
total purchase amount */

select sum(purch_amt) AS Total from orders;

/* Question 5 : Write a SQL query to find the highest purchase amount ordered by each customer.
...
/* Question 6 : Write a SQL query to calculate the average product price. Return average product
price. */

select round(AVG(purch_amt),2) AS Average_Product_Price from orders;
...
/* Question 7 : Write a SQL query to find those employees whose department is located at
...
/* Question 8 : Write a SQL query to find those employees whose salary is lower than that of
...
/* Question 9: Write a SQL query to find all those employees who work in department ID 80 or
...
/* Question 10 : Write a SQL query to calculate the average salary, the number of employees
receiving commissions in that department. Return department name, average
salary and number of employees. */

select d.department_name,
 AVG(e.salary) as Average_Salary,
 count(e.commission_pct) as employees_with_Commision
 from employees e
 join departments d
  on e.department_id = d.department_id
group by d.department_name;

/* Question 11: Write a SQL query to find out which employees have the same designation as the
...
/* Question 12: Write a SQL query to find those employees who earn more than the average salary.
...
/* Question 13: Write a SQL query to find all those employees who work in the Finance
...
/* Question 14: From the following table, write a SQL query to find the employees who earn less
...
/* Question 15: Create a stored procedure Count Employees ByDept that returns the number of
...
begin
select d.department_name, d.department_id, count(e.employee_id) as emp_count
from departments d 
...
/* Question 16: Create a stored procedure AddNewEmployee that adds a new employee to the
database. */
...
create procedure Add_emp(
    ...
    in ph_num int(12),
    in h_d date,
    in p_jobID varchar(20),
    in p_salary decimal,
    in p_commPCT decimal,
    in m_id int,
    in dep_ID int)
begin
insert into employees ( employee_id, first_name, last_name, email, phone_number,
hire_date, job_id, salary, commission_pct, manager_id, department_id)
values (e_id, f_name,l_name ,email ,ph_num ,h_d ,p_jobID, p_salary ,p_commPCT ,m_id ,dep_ID);
end //
delimiter ;


call Add_emp(207,'Bhaumik', 'Mandalia','bhaumiknimesh9@gmail.com', 94674791, '2025-09-23','DS',30000, NULL, 101,80);


/* Question 17: Create a stored procedure DeleteEmployeesByDept that removes all employees
...
/* Question 18: Create a stored procedure GetTopPaidEmployees that retrieves the highest-paid
employee in each department */

delimiter //

create procedure top_paid_emp()
BEGIN
...
call top_paid_emp();
/* Question 19 : Create a stored procedure PromoteEmployee that increases an employee’s salary
...
begin
  update employees
  set salary = salary * (1 + salary_increase/100.0),
  job_id = new_role
...
call promote_employee(114, "FI_Account", 20);

select*from employees;

/* Question 20 : Create a stored procedure AssignManagerToDepartment that assigns a new
...
BEGIN
  update employees
  SET manager_id = new_man_id
  where department_id = dep_id;
END //
delimiter ;

call Assign_manager(100,116);
